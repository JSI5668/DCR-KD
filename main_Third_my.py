...
######### last checkpoint 받아서 이어서 학습 ##############
        ...
        while True: #cur_itrs < opts.total_itrs:
            ...
            for (images, labels) in train_loader:
                ...
                preds = fake_segs.max(dim=1)[1].unsqueeze(1).float()  # 다중 채널 세그멘테이션 맵을 단일 채널로 변환
                output_D = discriminator(preds, labels.unsqueeze(1).float())
                loss_G_GAN = criterion_GAN(output_D, real_labels)  # 생성기가 판별기를 속이도록 훈련, output_D 는 (4, 1, 11, 11) 형태의 logits 값, real_labels 도 (4, 1, 11, 11) 형태의 1로 채워진 tensor 값
                loss_G_seg = criterion_seg(fake_segs, labels.squeeze(1))
                ...
                output_fake = discriminator(preds.detach(), labels.unsqueeze(1).float())  # detach()로 생성기와 그래프 연결을 끊음
                ...
                if (cur_itrs) % opts.val_interval == 0:
                    ...
                    save_ckpt_generator('F:/Third_paper/Checkpoint/Segmentation/SegGAN/CamVid_Firstfold/12.5_percent/lr_0.0001_0.1lossGAN/latest_%s_%s_os%d.pth' %
                              (opts.model, opts.dataset, opts.output_stride))
                    ...
                    if val_score['Mean IoU'] > best_score:  # save best model
                        ...
                        #           (opts.model, opts.dataset,opts.output_stride))
                        save_ckpt_generator('F:/Third_paper/Checkpoint/Segmentation/SegGAN/CamVid_Firstfold/12.5_percent/lr_0.0001_0.1lossGAN/best_%s_%s_os%d.pth' %
                                  (opts.model, opts.dataset, opts.output_stride))
                    ...
                    save_ckpt_generator('F:/Third_paper/Checkpoint/Segmentation/SegGAN/CamVid_Firstfold/12.5_percent/lr_0.0001_0.1lossGAN/_%s_%s_%s_os%d.pth' %
                ...
                if cur_itrs >= opts.total_itrs:
                    ...
                    df_train_miou.to_csv('F:/Third_paper/Checkpoint/Segmentation/SegGAN/CamVid_Firstfold/12.5_percent/lr_0.0001_0.1lossGAN/train_miou_2.csv', index=False)
                    df_train_loss.to_csv('F:/Third_paper/Checkpoint/Segmentation/SegGAN/CamVid_Firstfold/12.5_percent/lr_0.0001_0.1lossGAN/train_loss_2.csv', index=False)
