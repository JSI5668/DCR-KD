...
from network.Third_paper import SegmentationDiscriminator
...
def get_argparser():
    ...
    #                     help="restore from checkpoint")
    parser.add_argument("--ckpt",default='/workspace/hdd2/Third_paper/Checkpoint/Third_paper/Segmentation/Proposed_SegGAN/gener_smp_unetplusplus_resnetmy_2/Original/CamVid_Firstfold/best_deeplabv3plus_resnet50_camvid_sample_os16.pth', type=str,
                        help="restore from checkpoint")
...
def main():
    ...
    discriminator = SegmentationDiscriminator(input_channels=1)
    generator.cuda()
    discriminator.cuda()

    # optimizer_G = optim.Adam(generator.parameters(), lr=opts.lr)
    # optimizer_D = optim.Adam(discriminator.parameters(), lr=opts.lr)
    optimizer_G = optim.RMSprop(generator.parameters(), lr=opts.lr_G, weight_decay=1e-8, momentum=0.9)
    optimizer_D = optim.RMSprop(discriminator.parameters(), lr=opts.lr_D, weight_decay=1e-8, momentum=0.9)
...
######### last checkpoint 받아서 이어서 학습 ##############
        ...
        while True: #cur_itrs < opts.total_itrs:
            ...
            for (images, labels) in train_loader:
                ...
                real_labels = torch.ones(batch_size, 1, 11, 11).to(images.device)
                fake_labels = torch.zeros(batch_size, 1, 11, 11).to(images.device)
                ...
                for _ in range(g_steps):
                    ...
                    loss_G = (0.1 * loss_G_GAN) + loss_G_seg
                    # print("loss_GAN: ", (0.1 * loss_G_GAN))
                ...
                if (cur_itrs) % opts.val_interval == 0:
                    ...
                    save_ckpt_generator('F:/Third_paper/Checkpoint/Segmentation/SegGAN/CamVid_Firstfold/Original/smp_unetplusplus_resnetmy_2/gen0.0001_2_dis0.01_1/latest_%s_%s_os%d.pth' %
                              (opts.model, opts.dataset, opts.output_stride))
                    ...
                    if val_score['Mean IoU'] > best_score:  # save best model
                        ...
                        #           (opts.model, opts.dataset,opts.output_stride))
                        save_ckpt_generator('F:/Third_paper/Checkpoint/Segmentation/SegGAN/CamVid_Firstfold/Original/smp_unetplusplus_resnetmy_2/gen0.0001_2_dis0.01_1/best_%s_%s_os%d.pth' %
                                  (opts.model, opts.dataset, opts.output_stride))
                    ...
                    save_ckpt_generator('F:/Third_paper/Checkpoint/Segmentation/SegGAN/CamVid_Firstfold/Original/smp_unetplusplus_resnetmy_2/gen0.0001_2_dis0.01_1/_%s_%s_%s_os%d.pth' %
                ...
                if cur_itrs >= opts.total_itrs:
                    ...
                    df_train_miou.to_csv('F:/Third_paper/Checkpoint/Segmentation/SegGAN/CamVid_Firstfold/Original/smp_unetplusplus_resnetmy_2/gen0.0001_2_dis0.01_1/train_miou_2.csv', index=False)
                    df_train_loss.to_csv('F:/Third_paper/Checkpoint/Segmentation/SegGAN/CamVid_Firstfold/Original/smp_unetplusplus_resnetmy_2/gen0.0001_2_dis0.01_1/train_loss_2.csv', index=False)
