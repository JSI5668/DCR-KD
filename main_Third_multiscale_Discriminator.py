...
def create_labels(batch_size, scales, device):
    real_labels = [torch.ones(batch_size, 1, h, w).to(device) for h, w in scales]
    fake_labels = [torch.zeros(batch_size, 1, h, w).to(device) for h, w in scales]
    return real_labels, fake_labels


def multi_scale_loss(D, real_images, fake_images, real_labels, fake_labels, criterion):
    real_outputs = D(real_images, real_images)
    fake_outputs = D(fake_images, real_images)
    loss_D_real = 0
    loss_D_fake = 0

    for real_output, fake_output, real_label, fake_label in zip(real_outputs, fake_outputs, real_labels, fake_labels):
        real_label_resized = F.interpolate(real_label, size=real_output.size()[2:], mode='nearest')
        fake_label_resized = F.interpolate(fake_label, size=fake_output.size()[2:], mode='nearest')
        loss_D_real += criterion(real_output, real_label_resized)
        loss_D_fake += criterion(fake_output, fake_label_resized)

    loss_D_real /= len(real_outputs)
    loss_D_fake /= len(fake_outputs)
...
def main():
    ...
    discriminator = MultiScaleDiscriminator(input_channels=1)
    generator.cuda()
    discriminator.cuda()
...
######### last checkpoint 받아서 이어서 학습 ##############
        ...
        while True: #cur_itrs < opts.total_itrs:
            ...
            for (images, labels) in train_loader:
                ...
                scales = [(images.size(2), images.size(3)),
                          (images.size(2) // 2, images.size(3) // 2),
                          (images.size(2) // 4, images.size(3) // 4)]

                real_labels, fake_labels = create_labels(batch_size, scales, device)
                ...
                # Fake images and real images for each scale
                fake_segs = generator(images)
                preds = fake_segs.detach().max(dim=1)[1].unsqueeze(1).float()

                # Calculate multi-scale loss
                loss_D_real, loss_D_fake = multi_scale_loss(discriminator, labels.unsqueeze(1).float(), preds,
                                                            real_labels, fake_labels, criterion_GAN)
                ...
                loss_G_GAN = 0
                for output, real_label in zip(output_D, real_labels):
                    real_label_resized = F.interpolate(real_label, size=output.size()[2:], mode='nearest')
                    loss_G_GAN += criterion_GAN(output, real_label_resized)
                loss_G_GAN /= len(output_D) # 생성기가 판별기를 속이도록 훈련, (생성기가 판별기를 속여서 판별기가 이 출력을 진짜 이미지로 인식하도록 하려는 목표) output_D 는 (4, 1, 11, 11) 형태의 logits 값, real_labels 도 (4, 1, 11, 11) 형태의 1로 채워진 tensor 값
